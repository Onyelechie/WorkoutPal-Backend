...
func (u *userRepository) ReadUsers() ([]*model.User, error) {
	...
	for rows.Next() {
		var user model.User
		var avatarURL sql.NullString
	err := rows.Scan(&user.ID, &user.Username, &user.Email, &user.Name, &user.Age, &user.Height, &user.HeightMetric, &user.Weight, &user.WeightMetric, &avatarURL, &user.IsPrivate, &user.ShowMetricsToFollowers)
		if err != nil {
			return nil, err
		}
		if avatarURL.Valid {
...
func (u *userRepository) CreateUser(request model.CreateUserRequest) (*model.User, error) {
	var user model.User
		...
		RETURNING id, username, email, name, age, height, height_metric, weight, weight_metric, avatar_data, is_private, show_metrics_to_followers`,
				...
				&user.Height, &user.HeightMetric, &user.Weight, &user.WeightMetric, &user.Avatar, &user.IsPrivate, &user.ShowMetricsToFollowers)
...
func (u *userRepository) UpdateUser(request model.UpdateUserRequest) (*model.User, error) {
	...
	var avatarURL sql.NullString
		...
		WHERE id=$1 RETURNING id, username, email, name, age, height, height_metric, weight, weight_metric, avatar_data, is_private, show_metrics_to_followers`,
				...
				request.Height, request.HeightMetric, request.Weight, request.WeightMetric, request.Avatar, request.IsPrivate, request.ShowMetricsToFollowers).Scan(
				&user.ID, &user.Username, &user.Email, &user.Name, &user.Age,
				&user.Height, &user.HeightMetric, &user.Weight, &user.WeightMetric, &avatarURL, &user.IsPrivate, &user.ShowMetricsToFollowers)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, errors.New("user not found")
		}
